...
- [ ] **LA / Sentinel tiers as a Commit service** — proposed design (2026-07-06, not yet agreed with the pipeline side):
	...
	- `service`: `log_analytics` | `sentinel` — **two rows for one Sentinel-enabled workspace** (same scope_id). LA and Sentinel tiers are independent commitments sized on different volumes (LA = all ingestion; Sentinel = security-table subset). Keying on the workspace alone would overwrite one with the other.
