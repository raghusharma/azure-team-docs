...
- [ ] **LA / Sentinel tiers as a Commit service** — proposed design (2026-07-06, not yet agreed with the pipeline side):
	- `scope_type: "resource"` (new enum value) — a commitment tier binds to exactly **one workspace**, it doesn't pool. Pipeline must learn the value before the first LA file lands (we're pre-v1 in dev, no version bump).
	- `scope_id` = the workspace's **full ARM resource ID**, lowercased — deviates from the bare-ID rule deliberately: a workspace path isn't reconstructable from a constant prefix, and the "purchase" (tier change) is an ARM PATCH on that path.
	- `service`: `log_analytics` | `sentinel`; `normalized_sku_family: "commitment-tier"` (constant — scope_id already makes the row unique) — **two rows for one Sentinel-enabled workspace** (same scope_id). LA and Sentinel tiers are independent commitments sized on different volumes (LA = all ingestion; Sentinel = security-table subset). Keying on the workspace alone would overwrite one with the other.
